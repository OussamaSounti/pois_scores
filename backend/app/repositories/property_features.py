...
class PropertyFeaturesRepository:
    ...
    def count_pending_for_refresh(self, refresh_ts: datetime) -> int:
        """Alias for weekly incremental pending (``refresh_ts`` unused; API compat)."""
        return self.count_pending_weekly(refresh_ts)

    def count_pending_weekly(self, _current_poi: datetime) -> int:
        """Properties with no feature row yet — score once against current POI.

        Skips any transaction already present in ``property_features`` (e.g.
        historical backfill). ``_current_poi`` is kept for call-site compatibility.
        """
        sql = text(f"""
            SELECT count(*)
            FROM {T_PROPERTIES} p
        ...
        """)
        return int(self._session.execute(sql).scalar() or 0)

    def fetch_pending_weekly(
        self,
        _current_poi: datetime,
        limit: int,
