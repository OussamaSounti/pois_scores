...
class PropertyScoresOut(BaseModel):
    ...
    dist_coast_km: float | None = Field(
        default=None,
        ge=0,
        description="distance to nearest coastline (km) or None when geo.coastline is not loaded.",
    )
    land_buffer_fraction_1km: float | None = Field(
        ...
        le=1,
        description=(
            "Fraction of the 1 km buffer on land. Below 1.0 means the property "
            "is within 1 km of the sea."
        ),
    )
    ...
    poi_source: str | None = Field(
        default=None,
        description=(
            "'history' when scored from history, "
            "'current' when scored from active.production_pois_current."
        ),
    )
