...
@task
def count_pending_weekly(current_poi: datetime) -> int:
    session = get_session_factory()()
    try:
        return PropertyFeaturesRepository(session).count_pending_weekly(current_poi)
    finally:
        session.close()
...
def weekly_recompute_flow(chunk_size: int = CHUNK_SIZE) -> dict[str, str | int]:
    """Score newly appended properties against the current POI snapshot."""
    ...
    if external_ts is None:
        logger.warning(
            "No POI refresh timestamp found in active.audit_pipeline_runs. Skipping run."
        )
    ...
    pending_before = count_pending_weekly(external_ts)

    logger.info(
        "external_ts=%s pending_before=%d",
        external_ts,
        pending_before,
    )
    ...
    with PipelineRunTracker(
        ...
        if pending_before == 0:
            logger.info("No newly appended properties; skipping.")
            run.skip("no_pending", pending_after=0)
        ...
        pending_after = count_pending_weekly(external_ts)
