...
- :func:`weekly_recompute_flow` — scheduled weekly. Idempotent skip when
  there is no new external POI refresh AND no pending properties.
- :func:`historical_backfill_flow` — on-demand. Wraps
  :func:`run_historical_batch` in a Prefect-aware task for retries / logging.
"""
...
def get_latest_external_refresh() -> datetime | None:
    """Latest POI refresh timestamp from ``active.audit_pipeline_runs``."""
...
@task
def run_weekly_continuous_task(chunk_size: int = CHUNK_SIZE) -> int:
    return run_weekly_continuous(chunk_size=chunk_size)


@task
def run_historical_batch_task(chunk_size: int = HISTORICAL_CHUNK_SIZE) -> int:
    return run_historical_batch(chunk_size=chunk_size)
...
def weekly_recompute_flow(chunk_size: int = CHUNK_SIZE) -> dict[str, str | int]:
    ...
    processed_rows = run_weekly_continuous_task(chunk_size=chunk_size)
...
def historical_backfill_flow(
    ...
    logger = get_run_logger()
    processed = run_historical_batch_task(chunk_size=chunk_size)
