...
- Feature pipeline now uses `audit.pipeline_runs.run_timestamp` for POI versioning; `production.poi_imports` removed from schema and docs. Production DB is documented as existing and externally refreshed; dump/restore is local/dev only.
...
- Feature engineering pipeline: reads from `production.properties`, computes spatial indicators via shared logic, writes to `production.property_features`; POI version from `audit.pipeline_runs.run_timestamp`; run on demand or on schedule.
